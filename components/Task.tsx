    ...
    <TableRow ref={combinedRef} key={task.id}>
      ...
      <TableCell>
        <Select value={currentStatus} onValueChange={handleStatusChange}>
          <SelectTrigger
            className="w-[150px] h-[30px] shadow-none text-[12px] border-none"
          ...
          <SelectContent className="items-start">
            ...
            <SelectItem value={TaskStatus.COMPLETED}>
              <Badge
                variant={'outline'}
                className="text-green-400 m-2 shadow-md"
              >
                Completed
              </Badge>
            </SelectItem>
      ...
      <TableCell className="flex h-12 justify-end items-center">
        {permission?.permissions == 'READ' ? (
          <></>
        ) : (
          <DeleteTask taskId={task.id} projectId={projectId} />
        )}
      </TableCell>
    </TableRow>
  );
