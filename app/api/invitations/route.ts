  ...
  try {
    ...
    if (!id || !userEmail || !permissions) {
      return new NextResponse(
        JSON.stringify({
          success: false,
          error: 'Project ID, user ID, and permissions are required',
        }),
        { status: 400 }
      );
    }
    ...
    return new NextResponse(JSON.stringify({ success: true, invitation }), {
      status: 201,
    });
  } catch (error) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    return new NextResponse(JSON.stringify({ success: true, invitations }), {
      status: 200,
    });
  } catch (error) {
